"""
SQLite-specific models for PrivacyWeave Chatbot
These models mirror the PostgreSQL models but are optimized for SQLite
"""
...
class SQLiteChatLog(SQLiteBase):
...
class SQLiteRegister(SQLiteBase):
...
class SQLiteDataLeak(SQLiteBase):
...
class SQLiteSecurityScanResults(SQLiteBase):
